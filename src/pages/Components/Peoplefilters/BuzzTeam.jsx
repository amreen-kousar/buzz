...
export default function BuzzTeams({ selectDATA, getData }) {
    const [fund, setFund] = useState();
    useEffect(() => {
        console.log(selectDATA, "<--dffgdfgdfn ---- buzzTeam")
        if (selectDATA >= 0) {
            buzzTeam()
        }
    }, [selectDATA]
    )
    const buzzTeam = async () => {
        ...
        axios(config)
            .then((response) => {
                ...
                console.log(JSON.stringify(response.data), "buzz team response");
            })
            .catch((error) => {
                console.log(error);
            });
