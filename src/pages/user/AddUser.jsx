...
function AddUser(props) {
  ...
  const checkEmailExists = () => {
    console.log("emailChecking method calling ")
    ...
    axios(config)
      .then((response) => {
        if (response.status == 200) {
          console.log(response , "cemailhecking  ");
          setEmailExists(true);
        } else {
            console.log(response , "email checking faield ");
  ...
  const submitUser = () => {
    ...
    (AddUser.createdBy = userid), (AddUser.lastUpdatedBy = userid);
    console.log(AddUser);
    ...
    let apiCallName =
      AddUser.role.roleName == 'Funder' ? 'createFunder' : AddUser.role.roleName == 'Partner' ? 'createPartner' : false;
            ...
            >
              <div style={{ background: 'white', padding: '2rem', borderRadius: '10px' }}>
                <FormControl fullWidth style={{ marginLeft: '0.5rem', marginBottom: '0.5rem', color: '#ff7424' }}>
                  <InputLabel id="demo-simple-select-label" fullWidth color="common" style={{ color: '#ff7424' }}>
                    {AddUser.role.id > 1 ? 'Role' : 'Choose Role'}{' '}
                  </InputLabel>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      console.log(e.target.value , " role get emp id ")
                    }}
                  >
                    {roles.map((role) => {
                ...
                {[
                  ...
                  'Program Manager',
                  'Operations Manager',
                  'Gelathi Facilitator Lead',
                  'FIN/HR/VIEWER',
                  'Senior Operations Manager',
                ].includes(AddUser.role?.roleName) && (
                  <TextField
                    fullWidth
                    id="outlined-basic"
                    label="Last Name"
                ...
                {!['Funder', 'Partner'].includes(AddUser.role?.roleName) && (
                  <FormControl style={{ marginLeft: '1rem' }}>
                    <RadioGroup
                      row
                      onChange={(e, value) => {
                        setAddUser({ ...AddUser, gender: value });
                      }}
              ...
              <div style={{ background: 'white', padding: '2rem', borderRadius: '10px' }}>
                <TextField
                  ...
                  id="outlined-basic"
                  label="Mobile number"
                  helperText={`Mobile Number Required (${AddUser?.contactNum?.length}/10)`}
                  inputProps={{ maxLength: 10 }}
                  multiline
                  value={AddUser.contactNum}
                  type="number"
                  onChange={(e) => {
                ...
                <TextField
                  ...
                  id="outlined-basic"
                  label="Work"
                  value={AddUser.workNum}
                  onChange={(e) => {
                    setAddUser({ ...AddUser, workNum: e.target.value });
                  }}
                  type="number"
                  variant="outlined"
                  color="common"
                ...
                <div style={{ marginLeft: '1rem', fontSize: '0.8rem', fontWeight: '700' }}>
                  {emailExists ? (
                    <span style={{ color: 'crimson', display: 'flex' }}>
                      <Iconify icon="gridicons:cross-circle" width={20} height={20} /> &nbsp; Email Id already exists !
                    </span>
                  ) : errors.office_email_id ? (
                    <span style={{ color: 'crimson', display: 'flex' }}>
                      <Iconify icon="gridicons:cross-circle" width={20} height={20} /> &nbsp;Invalid Email Id
                    </span>
                  ) : AddUser.office_email_id != '' ? (
                    <span style={{ color: 'green', display: 'flex' }}>
                      <Iconify icon="mdi:tick-circle" width={20} height={20} /> &nbsp;Valid Email Id
                    </span>
                  ) : null}
                </div>
                ...
                {!['Funder', 'Partner'].includes(AddUser.role?.roleName) && (
                  <TextField
                    ...
                    id="outlined-basic"
                    label="Address 1"
                    value={AddUser.address3}
                    onChange={(e) => {
                      setAddUser({ ...AddUser, address3: e.target.value });
                    }}
                    variant="outlined"
                    color="common"
                  />
                ...
                {!['Funder', 'Partner'].includes(AddUser.role?.roleName) && (
                  <TextField
                    ...
                    id="outlined-basic"
                    label="Address 2"
                    value={AddUser.address2}
                    onChange={(e) => {
                      setAddUser({ ...AddUser, address2: e.target.value });
                    }}
                    variant="outlined"
                    color="common"
                  />
                ...
                <TextField
                  ...
                  color="common"
                />
                {['Trainer', 'Gelathi Facilitator', 'FIN/HR/VIEWER', 'Senior Operations Manager'].includes(
                  AddUser.role?.roleName
                ) && (
                  <FormControl fullWidth>
                    <Autocomplete
                      // disablePortal
                ...
                {['Driver'].includes(AddUser.role?.roleName) && (
                  <TextField
                    fullWidth
                    id="outlined-basic"
                    label="License Number"
                    value={AddUser.license_number}
                    onChange={(e) => {
                      setAddUser({ ...AddUser, license_number: e.target.value });
                    }}
                    variant="outlined"
                  />
                )}
